...
public class MOEAD extends AbstractMOEAD<DoubleSolution> {
  ...
  @Override public void run() {
    ...
    do {
      ...
      ArrayList<DoubleSolution> population_temp = new ArrayList<>(populationSize);
      ...
      for (int i = 0; i < populationSize; i++) {
        ...
        population_temp.add(child);
      }
      
      problem.evaluate(population_temp);
      evaluations+= populationSize;
      
      for (int i = 0; i < populationSize; i++) {
    	  
        DoubleSolution child = population_temp.get(i);
        int subProblemId = permutation[i];
        NeighborType neighborType = chooseNeighborType() ;
